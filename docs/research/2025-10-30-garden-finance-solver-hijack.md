...
## Technical Overview
Garden Finance utilizes an **Intent-Based Architecture**. In this model, users do not specify the exact steps of a trade (e.g., "call Uniswap, then bridge"). Instead, they sign an "intent" (e.g., "swap 100 ETH on Arbitrum for 100 ETH worth of BTC on Bitcoin"). "Solvers" (off-chain actors) compete to fulfill these intents, providing the requested liquidity in exchange for a fee.
