...
2. **Read-only Reentrancy:** Attackers exploited the fact that the contract's internal state (for example, liquidity balances) was mid-update when an external call was made. 
...
- **Heuristic:** Identifies functions sharing reentrancy keys (`@nonreentrant('lock')`) and verifies if any external calls (for example, `raw_call`, `ERC20.transfer`) occur before state finalization.
