...
4.  **Instant Liquidation:** The attacker immediately swapped these undervalued tokens on external liquidity pools (for example, PancakeSwap), extracting the underlying protocol value in a single block.
...
*   **Automatic Deviation Guards:** Implement sanity checks that revert transactions if the internal price deviates from a trusted external source by more than a set threshold (for example, 10%).
